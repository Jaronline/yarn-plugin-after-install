 ...
 */
export const executeAfterInstallModeHook = async (
  configuration: Configuration,
  printPreamble: boolean,
  mode?: boolean
): Promise<number> => {
  const afterInstall = configuration.get(mode ? 'afterInstallMode' : 'afterInstallNoMode')
...
/**
 * Execute the `afterInstallAlways` hook
 *
 * @param {Configuration} configuration Configuration
 * @param {boolean} printPreamble Whether to print a preamble before execution
 * @param {InstallMode} mode Wether an install mode has been used
